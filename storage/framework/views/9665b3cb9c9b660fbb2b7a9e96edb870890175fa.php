	...
	<div class="page-title">
		...
		<div class="page-breadcrumb">
			<ol class="breadcrumb">
				...
				<li class="active"><a href="<?php echo e(URL('/hrManager/teams')); ?>">Teams</a></li>
			</ol>
		</div>
	</div>
	<div id="main-wrapper">
							...
							<table class="table" id="datatable">
								...
								<tbody>
									...
									<tr>
										...
										<td><?php echo e(getUserById($team_leader->id)->first_name); ?> <?php echo e(getUserById($team_leader->id)->last_name); ?></td>
										<td><a href="javascript::void()" class="btn btn-primary" onclick="view(<?php echo e($team_leader->id); ?>)"><i class="fa fa-users"></i> View Team Members</a></td>
									</tr>
...
<script>
	...
	function remove(id){
		var teamMemberUrl = "<?php echo e(URL('/hrManager/remove/team-member/')); ?>";
	...
	function view(id){
		var member_name = '';
		var teamMemberUrl = "<?php echo e(URL('/hrManager/team-members/')); ?>";
		$.get(teamMemberUrl+'/'+id, function(data) {
			if(data.flag){
				$('#teamMembers').empty();
				$.each(data.team_members, function(index, team_member) {
					...
					$.get(userUrl+'/'+team_member.team_member_id, function(user_data) {
						...
						var html = `<tr id="member_`+user_data.user.id+`"><td>`+user_data.user.cb_profile.employee_id+`</td><td><img src="`+user_data.user.cb_profile.employee_pic+`" height="50" weight="50"></td><td>`+member_name+`</td><td>`+user_data.user.cb_profile.designation+`</td><td><a href="javascript::void()" class="btn btn-primary" onclick="remove(`+user_data.user.id+`)"><i class="fa fa-times"></i>Remove</a></td></tr>`;
